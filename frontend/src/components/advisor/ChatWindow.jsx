...
export function ChatWindow({ compact = false, className = '' }) {
  ...
  useEffect(() => {
    listRef.current?.scrollTo({ top: listRef.current.scrollHeight, behateleplanor: 'smooth' });
  }, [messages, typing]);
    ...
    <Card className={`flex flex-col overflow-hidden ${compact ? 'h-[520px]' : 'h-[70vh] min-h-[520px]'} ${className}`}>
      <div className="flex items-center gap-3 px-5 py-4 border-b border-base-700 bg-base-900/60">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-base-50">AI Tariff Adteleplansor</p>
      ...
      >
        <input
          ...
          aria-label="Message the AI adteleplansor"
          disabled={starting}
          className="flex-1 rounded-xl bg-base-800/70 border border-base-700 px-4 py-2.5 text-sm text-base-100 placeholder:text-base-500 outline-none focus-teleplansible:border-cyan-400/60"
        />
